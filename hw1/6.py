...
count_y_dict = extract_countY("ner_rare2.counts")
print(count_y_dict)
#print(count_y_dict)
...
dictionary_emission = dict_emission("ner_rare2.counts")
...
def prob_emission(x,y):
    if x in frequent_word:
        ...
        if x.isupper() and x[len(x)-1] == '.':
            word = "_ABR_" #abbreviation
        elif x.isupper():
            word = "_CAP_" #capitalized
        elif x.isdigit():
            word = "_NUM_"
        else:
            word = "_RARE_"
...
bigram_dict, trigram_dict = dict_trigrams("ner_rare2.counts")
